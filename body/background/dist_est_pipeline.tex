...
The DPT framework, introduced by Ranftl et al.\cite{dpt}, adapts the conventional convolutional
encoder/decoder architecture typically used for dense prediction tasks\cite{segmentation_CNN,invariant_CNNs}
...
remaining pixels being coloured black.
...
\[
    (m,c) \approx \argmin_{m,c} \sum_{i=1}^{N} |m \cdot median(m_i \cdot \textbf{D}_i^{ref}(\textbf{M}_i^{ref}=1) + c_i ) + c - \dfrac{1}{z_i}|
\]
...
calculated.
This is done by aligning the depth scale and shift of the raw depth map of the detection frame to
the overall calibration function derived for the camera location.
...
\]
respectively.\cite{HAUCKE2022101536}
