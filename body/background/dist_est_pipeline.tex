...
DPT, introduced by Ranftl et al.\cite{dpt}, is a, novel-conceived, architecture for dense prediction tasks,
adapting the conventional convolutional encoder/decoder architecture typically used for dense
prediction\cite{segmentation_CNN,invariant_CNNs}
...
global context of the image and high feature resolution through all transformer stages\cite{dpt}.
...
\[
    (m,c) \approx \argmin_{m,c} \sum_{i=1}^{N} |m \cdot \textbf{median}(m_i \cdot \textbf{D}_i^{ref}(\textbf{M}_i^{ref}=1) + c_i ) + c - \dfrac{1}{z_i}|
\]
...
the novelty of Depth Anything, however, lies in how it is trained.
...
To achieve this, the researchers first train a 'teacher' MDE model on approximately 1.5 million
pre-labelled monocular images and corresponding ground-truth targets collected from a variety of
datasets.
A "data-engine" is created to collect approximately 62 million unlabelled images from many datasets,
which are then automatically assign target depth labels by the teacher model.
