\subsection{Distance Estimation Pipeline}\label{subsec:dist_est}
...
or both Mega Detector and Segment Anything\cite{segment_anything} used collectively for detection and
pixel sampling to derive a final distance estimate.
...
\begin{figure}[H]
    ...
    average depth sacle of the calibration frames to give corresponding metric depth maps.
...
used for dense prediction\cite{segmentation_CNN,invariant_CNNs}
(i.e., the labelling of all pixels in an image), by substituting the encoder with a series of staged
vision transfor-mers.\cite{vision_transformer,transformer}
...
For the application to MDE in this pipeline, the model is trained on a large aggregate
...
\begin{figure}[H]
    ...
    \caption{Example of a DPT calibration frame (left) and corresponding binary mask (right) at 2 meters.
...
fitting values for scale and shift to minimise overall disparity error, the RANSAC fitting method
...
deformed, introducing perturbations\cite{depth_anything} such as colour distortion and spacial
distortion using CutMix.\cite{cutmix}
This necessitates that the model be responsive to more complex cues (e.g., geometry, high-level semantics)
in order to predict distances aligned with the predictions of the teacher model, trained on unperturbed
data.\cite{depth_anything}
The overall effect of this is that the model is more robust and generalised, showing strong performance in
zero-shot distance estimation.\cite{depth_anything,mde_models_comparison}

\subsubsection{Mega Detector}

Mega Detector is a deep learning animal (including human) detection model.
