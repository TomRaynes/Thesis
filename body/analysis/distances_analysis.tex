...
Observing the error graphs in Figures~\ref{fig:mae} and~\ref{fig:rmse}, it can be seen that all
configuration result in a significant spike in both mean average and root mean squared error at
...
What is not immediately explainable though is why this error appears do decline again (i.e., why
does the error not remain high?).
Camera trap are generally deployed in such a manner leave an area clear of foliage in front of the
...
\begin{figure}[H]
    ...
    \caption{Graphs showing mean DPT distance estimates (top) and all modelled distance
        ...
        shows the fitted regression line. The red dashed line shows the ideal (i.e., model=manual).
        the error bars show the 25–75 (green) and 5–95 (purple) percentiles. All distances are in
        units of meters. It can be seen that the spead of these estimates is greater than seen with
        the normally calibrated DPT. Estimates are also biased towards underprediction.}
