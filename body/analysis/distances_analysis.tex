...
This is because when multiple individuals are detected, there is ambiguity with regard to
...
The individual errors for each of these groups were then calculated.
Figure~\ref{fig:mae} shows the change in mean average error for each group while
...
\begin{figure}[H]
    ...
    estimates for the configurations: DPT/bounding box (top-left), DPT/segmentation (top-right),
        ...
        units of meters. It is seen that there is a reasonable correlation between manual and modelled
        distance, although the level of correlation varies between configurations.}
...
\begin{table}[H]
    ...
    \caption{Gradients of the regression lines (blue dotted lines in Figure~\ref{fig:distance_comparison})
        ...
        estimate for each configuration. These give a measure of how well the models capture the scale
        of depth of the images (benchmarked to the manual estimates), where a gradient of one captures
        perfect scale. Evidently, all values are lower than this, showing that the models do not capture
        the full scale of depth in the scene.}
...
\begin{figure}[p]
    ...
    \caption{Graphs showing all modelled distance estimates mapped to thier corresponding manual
        ...
        the ideal (i.e., model=manual). All distances are in units of meters. These graphs show the
        spread of distance estimates at each distance. By inspection, it can be seen that error in
        DPT estimates are generally spread mostly evenly above and below the ideal while Depth
        Anything estimates tend to be below the ideal. This aligns with the results seen in
        Table~\ref{tab:overall_errors}.}
...
\begin{figure}[H]
    ...
    estimates for each pipeline configuration. Generally, it is seen that MAE is high at the
    extremes of the distance scale with a spike at approximately seven meters.}
    \label{fig:mae}
    ...
    estimates for each pipeline configuration. Generally, it is seen that RMSE is high at the
    extremes of the distance scale with a spike at approximately seven meters.}
    \label{fig:rmse}
\end{figure}
...
Contrasting the distance estimates of the two detection methods using DPT distance estimation,
...
\begin{figure}[htbp]
    ...
    \caption{Example of DPT depth maps generated using bounding box (top) and segmentation
        ...
        while the segmentation method gives a distance estimate of 1.0 meters. Here, the
        bounding box detection method fails to calibrate to the scale of the
        reference frames.}
...
\begin{figure}[htbp]
    ...
    \caption{Example of Depth Anything depth maps generated using bounding box (top) and segmentation
        ...
        while the segmentation method gives a distance estimate of 4.17 meters. The distance
        estimates are close due to identical depth maps, unlike DPT where calibration is appied.}
...
\begin{figure}[H]
    \centering
    \makebox[0.8\textwidth][c]{
        \includegraphics[width=0.85\textwidth]{body/analysis/assets/depth_maps/occluded_bbox}
    }\\[1mm]
    \makebox[0.8\textwidth][c]{
        \includegraphics[width=0.85\textwidth]{body/analysis/assets/depth_maps/occluded_seg}
    }
    \caption{Example of DPT depth maps generated using bounding box (top) and segmentation
        ...
        gave a distance estimate of 6.96 meters. Here, the bounding box method results in
        underestimation as a result of occulding pixels in the bounding box.}
...
Additionally, the consequence of segmentation leading to a steeper regression line (relating
modelled and manual distance) may also explain the large increase in absolute error when comparing
Depth Anything's segmentation results to its bounding box results.
Here, this results in a lack of intersection between the segmentation regression line with
the ideal which indicates that, over all distances, the difference between the
corresponding modelled and manual data are greater than for the other configurations where
an intersection occurs.
This is also supported by noticing that the MAE and $\Delta_{average}$ for this configuration
are very close in value, implying that most estimates are over-predicted.
...
estimates are greater and also closer to the ideal (gradient = 1) than that of Depth Anything.
This shows that, overall, DPT is better at capturing the true scale of depth in the detection
...
\begin{figure}[H]
    \centering
    \makebox[0.8\textwidth][c]{
        \includegraphics[width=0.88\textwidth]{body/analysis/assets/depth_maps/fine_detail/bbox_dpt}
    }\\[1mm]
    \makebox[0.8\textwidth][c]{
        \includegraphics[width=0.88\textwidth]{body/analysis/assets/depth_maps/fine_detail/seg_dpt}
    }\\[1mm]
    \makebox[0.8\textwidth][c]{
        \includegraphics[width=0.67\textwidth]{body/analysis/assets/depth_maps/fine_detail/bbox_da}
    }\\[1mm]
    \makebox[0.8\textwidth][c]{
        \includegraphics[width=0.67\textwidth]{body/analysis/assets/depth_maps/fine_detail/seg_da}
    }
    \caption{Example of depth maps generated using DPT/BBOX (row one), DPT/SEG
        ...
        The depth maps show that Depth Anything is capturing the depth contours of fine structures
        better than DPT.}
...
For the segmentation method, however, there is no such influence since detection distance is
...
By observing the error graphs in Figures~\ref{fig:mae} and~\ref{fig:rmse}, it can be seen that all
configurations result in a significant spike in both mean average and root mean squared error at
approximately seven meters.
Of course, it is understandable that error in estimates will increase with distance.
Cameras have a finite resolution and distant (and occluded) objects will correspond to fewer
detected pixels in the image, meaning less information is captured.
This is analogous to human depth perception, i.e., more distant objects are more difficult to
distinguish.

What is not immediately explainable though is why this error appears to decline again (i.e., why
it does not remain high).
Camera traps are generally deployed in a manner such that an area is left clear of foliage in front of the
camera so that animals can actually be observed.
Detection outside of this clear area will, by definition, be occluded to some degree, with more
distant detections being more occluded.
A hypothesis here is that the increase in error at the spike is resulting from the increasing
occlusion with distance.
This, additionally, makes detection more difficult, meaning that at a certain distance, the only animals
detected will be ones that are clearly visible and therefore more likely to result in an accurate
distance estimate.
This would explain the spike; however, whether this is actually the case remains uncertain.

\clearpage

\subsubsection{Ablation Studies}\label{subsubsec:var_cal}
...
\begin{table}[htbp]
    ...
    model and manual estimates ($\Delta_{average}$) for the bounding box and segmentation detection methods
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{body/analysis/assets/distance_graphs/reduced_cal}
    \caption{Graphs showing mean DPT distance estimates (top) and all modelled distance
        estimates (bottom) mapped to their corresponding manual
        ...
        shows the fitted regression line. The red dashed line shows the ideal (i.e., model=manual).
        The error bars show the 25–75 (green) and 5–95 (purple) percentiles. All distances are in
        units of meters. It can be seen that the spread of these estimates is greater than seen with
        fully calibrated DPT. Estimates are also biased towards underprediction.}
...
in a biasing of distance estimates, with that largest error corresponding to the mid-distance
region (where no references were used).
This effect is visualised in Figure~\ref{fig:disparity_calibration}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{body/analysis/assets/calibration_effects/disparity_calibration}
    \caption{Simple visualisation showing how the number of reference frames can influence the fitting
        ...
        This shows how a lack of reference frames could bias the calibration.}
...
\begin{figure}[H]
    ...
    \caption{Two examples of DPT reference depth maps calibrated using all reference frames
        ...
        show a general biasing of depth towards closer distances, especially for pixels in the
        close to medium depth regions.}
...
\begin{figure}[H]
    ...
    \caption{Example of DPT reference depth maps calibrated using all reference frames (left) and reference
        frames at one and fifteen meters (right). The calibration seen on the right shows a loss of depth
        contrast in the far-distance region. Here, all pixels corresponding to distances of approximately ten
        meters and over are overestimated to distances greater than fifteen meters resulting in these estimates
        being clipped at this value.}
    \label{fig:far_scale}
\end{figure}
...
calibrated disparity significantly increases at large raw disparity values.
This is visualised in Figure~\ref{fig:disparity_scale}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{body/analysis/assets/calibration_effects/disparity_scale}
    ...
    different numbers of reference frames are used. The grey solid line represents the true mapping of raw
    to calibrated disparity (i.e, 1 / distance). The blue dashed
    line represents a linear disparity mapping, fitted using many reference frames
    while the red dashed line represents one fitted using only frames at one and fifteen meters.
    The intersection of the regression lines demonstrations the hypothesis of how short distances are
    underestimated and far distances are overestimated.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{body/analysis/assets/calibration_effects/MAE_var_cal}
    \caption{Graph showing the mean average error (MAE) of all distance estimates associated with
        a single camera location with respect to their corresponding manual distance estimates
        using DPT calibrated with a varying number of refrence frames.
        It is seen that, after approximetely ten reference frames, minimal change in
        error is occurring.}
...
\begin{figure}[H]
    ...
    \caption{DPT depth maps of a single camera location generatated using a variable number of
        ...
        distance of the furthest frame) in the bottom-right corner. In general, it is seen that
        with each susbsequent calibration, where an additional reference is added (i.e., at one meter
        further than the previous), the depth of more distant features can be captured.}
...
resulting in these pixels being capped at the maximum depth of fifteen meters.
...
The reason for this may be due to instability of the calibration function when few references are used
...
row in Figure~\ref{fig:variable_calibration}), a reference landmark is depicted as a segmented depth
in the depth maps.
...
This gives a calibration map where the landmark occupies the smallest number of pixels so as to maximise
the background context.
When few references are used, however, the furthest landmark is closer and occupies a prominent number
of pixels.
