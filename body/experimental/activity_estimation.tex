...
formatted and joined with the relevant location and camera trap video metadata so as to
satisfy the script's input requirements.
The final data must then be identical in structure to the manually annotated
...
\subsubsection{Applying Activity Script}

With the distance data and associated metadata now appropriately formatted, density
and abundance estimates were then derived using the WCF activity script.
For each dataframe, the script was adapted to apply appropriate truncation and binning
to the distance data in order to generate a smooth distance distribution histogram.\cite{distance_samping}
Preliminary runs of the script were then used to fit several detection functions to
this histogram, with the best fitting (lowest AIC value) being selected.\cite{distance_samping}